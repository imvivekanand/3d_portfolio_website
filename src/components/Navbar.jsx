...
const [toggle, setToggle] = useState(false);
  ...
  return (
    <nav className={`${styles.paddingX} w-full flex items-center py-5 fixed top-0 z-20 bg-primary`}>
      ...
      />
      <p className='text-white text-[18px] font-bold cursor-pointer flex'>Vivek Anand &nbsp;
      <span className='sm:block hidden' >| QualiMe</span></p>
      </Link>
      <ul className="list-none hidden sm:flex flex-row gap-x-10">
      {navLinks.map((Link) => (
